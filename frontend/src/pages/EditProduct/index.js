...
const EditProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length > 0) {
      ...
      try {
        ...
        const response = await apiCalls.editProduct(formData, product.id);
  ...
  return (
    <>
      <MetaDecorator title="Editar Producto - iBoo Challenge" />
      <Layout>
        {console.log(form)}
        <section className={editStyles.container}>
          <h3 className={editStyles.title}>Editar producto</h3>
          <Form className={editStyles.form}>
            ...
            <Form.Group className="mb-3" controlId="enabled">
              ...
              <Form.Select
                className={!!errors.enabled && `${editStyles.redBorder}`}
              ...
              <div className={editStyles.red}>{errors.enabled}</div>
            </Form.Group>

            <Form.Group className="mb-3" controlId="productType">
              <Form.Label>Categoría</Form.Label>
              <Form.Select
                className={!!errors.productType && `${editStyles.redBorder}`}
                placeholder="Selecciona estado"
                required
                throwInvalidValueError={true}
                valueAs="id"
              ...
              >
                ...
                {types &&
                  types.map((type, i) => {
                    return (
                      <option value={type.id} key={`type_${i}`}>
                        {type.name}
                      </option>
                    );
                  })}
              </Form.Select>
              <div className={editStyles.red}>{errors.productType}</div>
            </Form.Group>

            <Form.Group controlId="image" className="mb-3">
              <Form.Label>Adjunte una imagen si desea actualizarla</Form.Label>
              <Form.Control
                type="file"
                name="image"
